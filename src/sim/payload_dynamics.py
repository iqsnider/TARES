...
from sim.config import (MASS_TOTAL, MASS_DRONE, MASS_PAYLOAD, GRAVITY, J,
                        TETHER_LEN, D, K_M)
...
class PositionController:
    """
    Returns acceleration from a spring-mass damper system
    """
    ...
    def compute_u(self, x, p_pl_ref, v_pl_ref, a_ref=None):
        p_s = np.asarray(p_pl_ref) + np.array([0, 0, TETHER_LEN])
        v_s = np.asarray(v_pl_ref)
